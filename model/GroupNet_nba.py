...
from skew_normal_class import DiagSkewNormal, Top1DiagSkewNormal, DiagNormal
...
class FutureEncoder(nn.Module):
    def __init__(self, args,in_dim=4):
        ...
        elif args.ztype == 'skew':
            self.qz_layer = nn.Linear(self.out_mlp.out_dim, 3 * self.args.zdim)
        elif args.ztype == 'top1skew':
            self.qz_layer = nn.Linear(self.out_mlp.out_dim, 3 * self.args.zdim + 1)
        else:
            raise ValueError('Unknown hidden distribution!')
...
class GroupNet(nn.Module):
    def __init__(self, args, device):
        ...
        if args.ztype == 'gaussian':
            ...
            self.dist_class = DiagSkewNormal
            self.pz_layer = nn.Linear(scale_num*self.args.hidden_dim, 3 * self.args.zdim)
        elif args.ztype == 'top1skew':
            self.dist_class = Top1DiagSkewNormal
            self.pz_layer = nn.Linear(scale_num*self.args.hidden_dim, 3 * self.args.zdim + 1)
        else:
            raise ValueError('Unknown hidden distribution!')
        if args.learn_prior:
    ...
    def init_dist(self, logits):
        if 'topk' in self.args.ztype:
            dist = self.dist_class(logits=logits, k=self.k)
        else:
    ...
    def calculate_loss_kl(self,qz_distribution,pz_distribution,batch_size,agent_num,min_clip):
        if self.args.ztype == 'gaussian':
            ...
            loss = kl_divergence(qz_distribution, pz_distribution).sum()
    ...
    def forward(self,data):
        ...
        elif self.args.ztype in ['skew', 'top1skew', 'topkskew']:
            qz_distribution = self.init_dist(logits=qz_param)
        else:
            ValueError('Unknown hidden distribution!')
        if self.zero_alpha:
            qz_distribution.alpha = torch.zeros_like(qz_distribution.alpha)
        ...
        if self.args.learn_prior:
            ...
            if self.args.ztype == 'gaussian':
                ...
                pz_distribution = self.init_dist(logits=pz_param)
            else:
                ValueError('Unknown hidden distribution!')
        else:
            if self.args.ztype == 'gaussian':
            ...
            elif self.args.ztype in ['skew', 'top1skew']:
                pz_distribution = DiagNormal(mu=torch.zeros(past_feature.shape[0], self.args.zdim).to(past_traj.device), sigma=torch.ones(past_feature.shape[0], self.args.zdim).to(past_traj.device))
            else:
                ValueError('Unknown hidden distribution!')
        ...
        if self.args.learn_prior:
            ...
            if self.args.ztype == 'gaussian':
                ...
                pz_distribution = self.init_dist(logits=p_z_params)
            else:
                ValueError('Unknown hidden distribution!')
        else:
            past_feature_repeat = past_feature.repeat_interleave(sample_num, dim=0)
            if self.args.ztype == 'gaussian':
                ...
                pz_distribution = DiagNormal(mu=torch.zeros(past_feature_repeat.shape[0], self.args.zdim).to(past_traj.device), sigma=torch.ones(past_feature_repeat.shape[0], self.args.zdim).to(past_traj.device))
            else:
                ValueError('Unknown hidden distribution!')
        if self.zero_alpha:
            pz_distribution.alpha = torch.zeros_like(pz_distribution.alpha)
        pz_sampled = pz_distribution.rsample()
        # z = pz_sampled

        diverse_pred_traj,_ = self.decoder(past_feature_repeat,pz_sampled,batch_size,agent_num,past_traj,cur_location,sample_num=self.args.sample_k,mode='inference')
